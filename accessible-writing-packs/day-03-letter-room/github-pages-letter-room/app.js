(() => {
  const config = window.LETTER_ROOM_CONFIG || {};
  const $ = (selector) => document.querySelector(selector);
  const key = 'letter-room-pages-settings';
  ...
  body.dataset.profile=saved.profile||config.profile||'large';font.value=saved.scale||100;line.value=saved.line||180;contrast.checked=!!saved.contrast;motion.checked=!!saved.motion;apply();
