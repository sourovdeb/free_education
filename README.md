# Free Education Repository

**Made by Claude Code | 50+ lessons + 5 AI tools + 1 illustrated essay**
**Made by Claude Code | 50+ lessons + 5 AI tools + 1 illustrated psychology read**

---

## 🚀 Quick Start (Pick One)

### 💡 I want to teach English
👉 Go to **`routines/01_elt365_lessons_routine/`**
- 50 ready-made lesson plans
- Topics: Listening, Reading, Professional Development, Young Learners
- Format: Markdown (easy to copy/edit)
- Status: All drafted and ready to use

👉 [**Full Lesson Index**](routines/01_elt365_lessons_routine/LESSON_INDEX.md)

---

### 🔧 I want to use the AI tools
👉 Go to **`routines/02_python_toolkit_routine/`**
- 5 offline AI tools (no cloud, no API keys)
- Transcribe audio → text
- Extract text from PDFs
- Scrape web pages
- Organize files with AI
- Format: Python scripts (PyQt6 UI)

👉 [**Setup Instructions**](routines/02_python_toolkit_routine/README.md)

---

### 🧠 I want fact-checked psychology/mental-health claims
👉 Go to **`routines/03_psychology_claims_audit_routine/`**
- Cited, funding-disclosed audits of psychology, neuroscience, marketing, and mental-health claims
- Format: Markdown micro-blog posts (200–300 words) with full source lists

👉 [**Routine README**](routines/03_psychology_claims_audit_routine/README.md)
### 🧠 I want to read about human psychology & nature
👉 Go to **`routines/03_human_nature_young_readers/`**
- Illustrated ~10-minute read for teens/young adults
- Freud, Jung, Dostoevsky, and the Buddha on human nature — through the lens of evolutionary mismatch
- Doodled SVG diagrams + a sourced, honesty-flagged confidence table
- Format: standalone HTML (with diagrams) + plain Markdown companion

👉 [**Read it**](routines/03_human_nature_young_readers/human_nature_and_behavior.md)
### 🧠 I want a cited psychology/marketing claim audit
👉 Go to **`routines/03_psychology_marketing_audit_routine/`**
- Short, cited fact-checks of psychology, neuroscience, marketing, and mental-health-treatment claims
- Verifies primary sources, funding/conflicts of interest, and opposing research
- Format: Markdown (news items)
- Status: Active, first item published 2026-07-07

👉 [**Latest Audit**](routines/03_psychology_marketing_audit_routine/2026-07-07_antidepressants-vs-placebo-audit.md)
- Status: Active, latest item published 2026-07-10

👉 [**Latest Audit**](routines/03_psychology_marketing_audit_routine/2026-07-10_ssri-vs-therapy-audit.md)
### 🧠 I want to understand human nature
👉 Go to **`routines/03_human_nature_routine/`**
- A 10-minute illustrated read on why people do what they do
- Freud, Jung, Dostoevsky, and Buddha, in plain language
- Connects ancient psychology to habits, fear, anger, addiction, tribalism, politics, and more
- Format: Markdown + doodles (SVG)

👉 [**Read the Field Guide**](routines/03_human_nature_routine/HUMAN_NATURE_Field_Guide.md)

---

## 📂 What's Inside

| Folder | What | Items |
|--------|------|-------|
| **routines/** | Organized by Claude routine type | 61+ items |
| **routines/** | Organized by Claude routine type | 62+ items |
| **routines/** | Organized by Claude routine type | 61+ items |
| **routines/** | Organized by Claude routine type | 65+ items |
| **elt365_lessons/** | Original lessons (backup) | 50 lessons |
| **python_toolkit/** | Original tools (backup) | 5 tools |

---

## 🗺️ Navigation Map

```
📍 You are here (Free Education)
├─ 💬 Need detailed index?
│  └─ Read: MASTER_INDEX.md
│
├─ 🎓 Teaching content?
│  └─ Enter: routines/01_elt365_lessons_routine/
│     └─ Read: LESSON_INDEX.md
│
├─ 🛠️ AI tools setup?
│  └─ Enter: routines/02_python_toolkit_routine/
│     └─ Read: README.md
│
└─ 🧠 Psychology / human nature reading?
   └─ Enter: routines/03_human_nature_young_readers/
      └─ Read: human_nature_and_behavior.md (or the .html version for diagrams)
└─ 🧠 Human nature read?
   └─ Enter: routines/03_human_nature_routine/
      └─ Read: HUMAN_NATURE_Field_Guide.md
```

---

## 📊 By the Numbers

- **50** English lessons (ELT365, Pro Dev, Young Learners)
- **5** Python AI tools (100% offline)
- **1** illustrated psychology essay for young readers
- **1** illustrated psychology field guide (with 5 original doodles)
- **3** Claude Code routines
- **1** Publisher script (publish to WordPress)
- **1** Shared NLP utility module

---

## ⚡ Common Tasks

### Run an AI tool (Python)
```bash
cd routines/02_python_toolkit_routine/
python ai_file_organizer_pro.py    # Organize files with AI
python audio2txt.py                # Transcribe audio
python pdf2txtv2.py                # Extract from PDFs
python webscrapper.py              # Scrape websites
```

**First time?** Install dependencies:
```bash
pip install PyQt6 transformers torch pandas requests beautifulsoup4 yt-dlp
```
[See full requirements →](routines/02_python_toolkit_routine/README.md)

---

### Copy a lesson
1. Open `routines/01_elt365_lessons_routine/`
2. Find the lesson file you want
3. Copy the Markdown text
4. Use in your doc or learning platform

---

### Publish lessons to WordPress
```bash
cd routines/01_elt365_lessons_routine/publisher/
python elt365_lessons_publisher.py
```
Publishes all lessons as drafts to sourovdeb.com

---

### Read the human nature essay
Open `routines/03_human_nature_young_readers/human_nature_and_behavior.html` in any browser for the illustrated version, or read `human_nature_and_behavior.md` for the plain-text version.
### Read the Human Nature Field Guide
1. Open `routines/03_human_nature_routine/HUMAN_NATURE_Field_Guide.md`
2. Read top to bottom (about 10 minutes) — doodles render inline on GitHub
3. Check the "Gaps in this research" section at the end for follow-up sources

---

## 🎯 Features at a Glance

### ELT365 Lessons
✅ 30 receptive skills lessons (listening + reading)  
✅ 10 professional development lessons  
✅ 10 young learners lessons  
✅ Mapped to WordPress CMS  
✅ Full lesson index included  

### Python AI Toolkit
✅ 100% offline (no cloud API)  
✅ Auto-download models from HuggingFace  
✅ Optional Ollama integration  
✅ Dark-theme PyQt6 UI  
✅ Batch processing support  
✅ Markdown output for all tools  

### Human Nature for Young Readers
✅ Freud, Jung, Dostoevsky, Buddha — synthesized, not just summarized  
✅ Doodled SVG diagrams (iceberg, shadow, freedom fork, three poisons, history clock)  
✅ Every claim confidence-flagged (solid / interpretive / speculative)  
✅ HTML + Markdown formats  
### Human Nature Field Guide
✅ Four thinkers (Freud, Jung, Dostoevsky, Buddha) in plain language  
✅ Evolutionary/hunter-gatherer mismatch theory explained  
✅ 5 original SVG doodles  
✅ Covers habits, fear, anger, greed, sex, competition, religion, politics, innovation  
✅ Challenges good/bad binary thinking  
✅ Cited sources + a "gaps in research" section  

---

## 📖 Detailed Docs

| Document | For |
|----------|-----|
| [**MASTER_INDEX.md**](MASTER_INDEX.md) | Overview of ALL routines + items |
| [**LESSON_INDEX.md**](routines/01_elt365_lessons_routine/LESSON_INDEX.md) | All 50 lessons listed by category |
| [**Python Toolkit README**](routines/02_python_toolkit_routine/README.md) | Tool setup, usage, features |
| [**Human Nature Essay**](routines/03_human_nature_young_readers/human_nature_and_behavior.md) | Freud/Jung/Dostoevsky/Buddha for young readers |
| [**Human Nature Field Guide**](routines/03_human_nature_routine/HUMAN_NATURE_Field_Guide.md) | Psychology primer for young readers |

---

## 💾 Storage & Backups

All files exist in two places for safety:

```
✅ Original (backward compatible):
   ├─ elt365_lessons/
   └─ python_toolkit/

✅ Organized by routine:
   └─ routines/
      ├─ 01_elt365_lessons_routine/
      ├─ 02_python_toolkit_routine/
      └─ 03_human_nature_young_readers/
      └─ 03_human_nature_routine/
```

---

## 🤔 Lost? Need Help?

1. **Quick overview?** → Read this file (you're reading it!)
2. **Find a lesson?** → See [LESSON_INDEX](routines/01_elt365_lessons_routine/LESSON_INDEX.md)
3. **Use a tool?** → See [Python README](routines/02_python_toolkit_routine/README.md)
4. **Read the psychology essay?** → See [Human Nature Essay](routines/03_human_nature_young_readers/human_nature_and_behavior.md)
4. **Understand human nature?** → See [Human Nature Field Guide](routines/03_human_nature_routine/HUMAN_NATURE_Field_Guide.md)
5. **See everything?** → Check [MASTER_INDEX](MASTER_INDEX.md)

---

## 📜 License

CC0-1.0 (Public Domain) — Use freely.

---

**Last Updated:** 2026-07-05  
**Last Updated:** 2026-07-10  
**Generated by:** Claude Code
